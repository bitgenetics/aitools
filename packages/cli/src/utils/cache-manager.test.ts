...
function makeTmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ai-tools-cache-'));
}
...
describe('CacheManager.store', () => {
  ...
  it('writes tarball files under <agentsDir>/<file.dest>', () => {
    const entry = cache.store(MANIFEST.name, MANIFEST.version, TARBALL, MANIFEST);
    const expectedFile = path.join(entry.agentsDir, 'my-skill', 'SKILL.md');
