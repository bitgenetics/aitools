...
// Bundled content from tools/create-ai-tool ù regenerated by bundling the source files.
...
description: >-
  ...
  the aitools registry ù even if the user doesn't explicitly say "registry" or
...
For a **skill** ù create \`SKILL.md\` with required YAML frontmatter:
...
The \`name\` value **must exactly match the folder name**. This is how agents discover the skill ù a mismatch causes it to never activate.
...
- \`disable-model-invocation: true\` ù slash-command-only, never auto-loads (VS Code + Cursor)
- \`user-invocable: false\` ù hide from \`/\` menu but keep auto-loading (VS Code only)
- \`argument-hint\` ù hint shown in chat input when invoked as \`/skill-name\` (VS Code only)

For a **subagent** ù create a \`.md\` file with YAML frontmatter (name, description, tools list) and sections for Role, Responsibilities, Rules, Output Format.

For a **prompt** ù create a \`.md\` file with the reusable instruction or template.

For an **mcp-tool** ù the manifest needs an \`mcpServer\` block. Content files are optional. Read [references/manifest-reference.md](references/manifest-reference.md) for the \`mcpServer\` field spec.

### Step 2: Initialise the manifest

\`\`\`bash
cd my-skill/
aitools manifest init          # interactive ù prompts for all fields
\`\`\`
...
- \`dest\` accidentally prefixed with the category dir (don't ù the installer adds it)
...
- ? \`"dest": "skills/my-skill/SKILL.md"\` ù double-prefixed, wrong
...
- **Imperative**: "Use this skill whenù" not "This skill doesù"
- **Cover indirect triggers**: mention what the user is trying to achieve, not just the domain name ù "even if they don't explicitly say 'registry'"
- **Be specific**: list the concrete operations the skill covers
- **Hard limit**: 1024 characters

Before: \`"Create and publish tools."\` ù too vague, won't activate.

After: \`"Use this skill when asked to create, package, or publish a reusable AI tool ù even if the user doesn't say 'registry'. Covers skills, subagents, prompts, and MCP tools."\` ù activates on intent.
...
- **\`repository\` in the manifest must be a full URL.** \`user/repo\` fails validation ù use \`https://github.com/user/repo\`.
- **Run \`manifest validate\` before every publish** ù catches missing files before they reach the registry.
...
Read [references/platform-paths.md](references/platform-paths.md) if you need to know exactly where files land per platform (vscode, claude, cursor, windsurf, universal) ù useful when writing \`dest\` values or advising users on where tools install.
`;

export const MANIFEST_REFERENCE_MD = `# aitools.manifest.json ù Full Field Reference
...
| \`repository\` | string (URL) | Must be a full URL ù \`https://github.com/user/repo\`, not \`user/repo\` |
...
\`\`\`jsonc
{
  ...
  "template": true              // optional ù enables Handlebars {{variable}} substitution
}
\`\`\`

\`dest\` rule: the installer prepends the category directory automatically.
- ? \`"dest": "my-skill/SKILL.md"\` ? \`.agents/skills/my-skill/SKILL.md\`
- ? \`"dest": "skills/my-skill/SKILL.md"\` ù do not repeat the category dir
...
\`\`\`jsonc
{
  ...
  "url": "https://ù/mcp",        // remote HTTP server (overrides command)
...
## Full example ù mcp-tool

\`\`\`json
{
  "name": "@acme/postgres-mcp",
  "version": "1.0.0",
  "description": "MCP server for PostgreSQL ù run queries, inspect schema",
  "category": "mcp-tool",
  "mcpServer": {
    "command": "node",
    "args": ["server.js"],
...
|---|---|---|---|---|
| **universal** | \`.agents/skills/\` | \`.agents/agents/\` | \`.agents/prompts/\` | ù |
...
- For skills, the folder name in \`dest\` **must match the \`name\` field in the \`SKILL.md\` frontmatter** ù that's how agents find the skill file.
