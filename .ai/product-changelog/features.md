...
**Impact**: Unit tests cover JSONC parse, path resolution, argv building, dry-run, unknown-option forwarding, and Windows quoting; spawn is injectable. E2e: `packages/e2e/src/cursor-workspace.test.ts` asserts CLI dry-run emits `--workspace` + `--add-dir` for multi-root folders. CI builds/publishes `@bitgenetics/aitools-cursor` with the workspace (`51d2a0b`, `8c9ede8`).  
...
**Key files**: `packages/cursor/src/`, `packages/cli/src/commands/cursor.ts`, `packages/cli/src/commands/cursor.test.ts`, `packages/e2e/src/cursor-workspace.test.ts`
