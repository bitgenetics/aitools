...
description: >-
  ...
  aitools CLI. Use when asked to add a command, create a CLI feature, or
  expose new functionality via the aitools binary.
metadata:
  project: aitools
---
...
| `CacheManager` | `../utils/cache-manager.js` | Reads/writes the local package cache at `~/.aitools/cache/`. |
...
- **Do not read config manually** — `ConfigManager` handles project vs. global config cascade. Reading `aitools.config.json` directly bypasses merging.
