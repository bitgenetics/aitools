...
### Coverage targets
- **Statements / branches / functions**: aim for ≥ 80% on `@ai-tools/core` and `@ai-tools/cli`.
- The `@ai-tools/server` route handlers are integration-tested via `buildApp()` using Fastify's `inject()` method — no real HTTP port needed.
